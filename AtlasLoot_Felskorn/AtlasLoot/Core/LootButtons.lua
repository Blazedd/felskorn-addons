            ...
            --Default game tooltips
                ...
                if(this.itemID ~= nil) then
                    if(GetItemInfo(this.itemID) ~= nil) then
                        ...
                        ModifyAtlasLootTooltip(AtlasLootTooltip, this.itemID)
...
local function FindItemIDByNameInLeft2AndApplyFlag(tooltip)
    ...
    if leftLine2 then
        local itemName = leftLine2:GetText()
        if itemName then
            local itemID = select(1, GetItemInfo(itemName))
            if itemID then
                    ...
                    if flag then
                        ...
                        for j = 1, tooltip:NumLines() do
                            ...
                            if leftLineHeroic and leftLineHeroic:GetText() == "Heroic" then
                                leftLineHeroic:SetText(lineText)
                                return
                            end
                        end
                    end
