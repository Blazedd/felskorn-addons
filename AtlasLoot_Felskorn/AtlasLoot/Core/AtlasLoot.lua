...
ATLASLOOT_VERSION = "|cffFF8400AtlasLoot |r";
EDIT_BY = "Felskorn"
...
function AtlasLoot_GetLODModule(dataSource)
	...
	elseif (dataSource=="AtlasLootFelskorn") then
		return "AtlasLoot_Felskorn";	
	end
end
...
function AtlasLoot_LoadAllModules()
	local orig, bc, wotlk, craft, world, felskorn;
    ...
    felskorn, _ = LoadAddOn("AtlasLoot_Felskorn");
    local flag=0;
	if not orig then
    ...
    if not felskorn then
		LoadAddOn("AtlasLoot_Felskorn");
		flag=1;
	end
	if flag == 1 then
		if ATLASLOOT_DEBUGMESSAGES then
